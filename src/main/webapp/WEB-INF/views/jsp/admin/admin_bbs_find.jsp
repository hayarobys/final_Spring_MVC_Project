...
<div id="article"> 
  ...
  <div id="article_c"> 
   ...
   <div id="aMain_cont">
     <div id="aGongji_list">
      <h2 class="aGlist_title">관리자 자료실 검색</h2>
		<div id="aGlist_count">
			총게시물 수 : ${listcount}<br />
		</div>
...
클릭하면 삭제를 안하고 그대로 현재창에 있게 된다. --> 				
		...
		<div id="aGlist_paging">
			...
			[이전]&nbsp;
			<%}else{ %>
			<a href="admin_bbs_find.do?page=<%=nowpage-1 %>&find_name=${find_name}&find_field=${find_field}" onfocus="this.blur();">[이전]</a>&nbsp;
			<%} %>
			...
			[<%=a %>]
			<%}else{ %>
			<a href="admin_bbs_find.do?page=<%=a %>&find_name=${find_name}&find_field=${find_field}" onfocus="this.blur();">[<%=a %>]
			</a>&nbsp;
			<%} %>
			<%} %>
			...
			[다음]
			<%}else{ %>
			<a href="admin_bbs_find.do?page=<%=nowpage+1 %>&find_name=${find_name}&find_field=${find_field}" onfocus="this.blur();">[다음]</a>
			<%} %>
		</div>
		<div id="aGlist_menu">
		      ...
		      <table id="aGlistFind_t">
		        <tr>
		         ...
		         <select name="find_field">
		          ...
		          ${'selected'}</c:if>>자료실제목</option>
		          <option value="bbs_content"
		          <c:if test="${find_field == 'bbs_content'}">
		          ${'selected'}</c:if>>자료실내용</option>		          	          
		         </select>
